...
def check_abaqus_connection() -> str:
    ...
    ts = float(status.get('timestamp', 0) or 0)

    if ts and time.time() - ts > 15.0:
        age = int(time.time() - ts)
        return (f'Abaqus plugin status is stale ({age}s old). '
                f'The Abaqus MCP worker is not responding.\n'
                f'Status: {s} - {msg}\nLast update: {dt}\n'
                f'Use Plug-ins -> MCP -> Start MCP (Cooperative) or Start MCP (Blocking).')
