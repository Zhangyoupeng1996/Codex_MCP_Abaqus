...
def _to_text(value):
    """Return a text/unicode string for io.open(..., encoding=...) writes."""
...
def write_status(status, message=""):
    ...
    try:
        with io.open(tmp_file, 'w', encoding='utf-8') as f:
            f.write(_to_text(json.dumps(payload, indent=2, ensure_ascii=False)))
        for _ in range(5):
            try:
...
def _write_json(path, data):
    with io.open(path, 'w', encoding='utf-8') as f:
        f.write(_to_text(json.dumps(data, indent=2, ensure_ascii=False)))
...
def execute_script(script_content, script_id):
    ...
    try:
        with io.open(script_path, 'w', encoding='utf-8') as f:
            f.write(_to_text(script_content))
    ...
    exec_globals['print'] = lambda *a, **k: output_lines.append(
        _to_text(' '.join([str(x) for x in a]))
    )
    ...
    try:
        ...
        try:
            with io.open(err_path, 'w', encoding='utf-8') as f:
                f.write(_to_text(str(e) + '\n\n'))
                f.write(_to_text(traceback.format_exc()))
        except Exception:
            pass
...
def mcp_start(interval=0.1):
    ...
    if not _background_self_test(timeout=1.5):
        ...
        write_status('error', 'Background self-test failed; use Cooperative or Blocking mode')
    else:
